---
title: 'STA 108 Discussion 7: Multiple linear regression'
author: "Yuanyuan Li"
output:
...
$$\text{s.e.}(\hat{\beta}_k)=\sqrt{MSE(X'X)^{-1}_{k,k}}, \ 0 \leq k \leq p-1,$$
where $(X'X)^{-1}_{k,k}$ is the $(k+1)$th diagonal element of $(X'X)^{-1}$, where the index starts from 1.
...
```{r}
CDI = read.table("datasets/CDI.txt")
